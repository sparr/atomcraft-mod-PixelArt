...
| `PixelFont` | three hand-drawn bitmap fonts, 3x5, 5x7 and 7x11, all with real descenders, exact at any whole-number scale |
...
**Text is a bitmap font, on purpose.** A pixel is 8 world units, which is 12 screen pixels at the shipped game's closest zoom, and every scalable font blurs at that size because a glyph outline rasterized at 12px lands on fractional pixel boundaries. A bitmap font drawn at a whole-number scale from a whole-number position through a nearest-neighbour filter is exact instead. There are three sizes rather than one scaled three ways, because a 3x5 glyph magnified is still a 3x5 glyph: 7x11 buys round bowls, real diagonals and a deeper descender. Every size has descenders, drawn into the vertical spacing below the box rather than inside it, so caps get the whole box and a line of text is no taller for them.
...
- **`LabelLayout` can break inside a word, if you ask.** By default it wraps only at spaces, so a one-word name has exactly one arrangement -- itself, on one line -- and is limited by its own length rather than by the cell. A square cell usually has height going spare: on a 32-screen-pixel cell one line of `Water` fits a glyph height of 5 where `Wat` over `er` fits 11, and on a 16-pixel cell the one-line rule gives up and shows `W` and an ellipsis where two lines show the whole name. Pass `Breaking.Anywhere` to allow the split. Breaking a word is priced rather than free -- a split candidate must be more than `(1 + wordSplitCost) ^ breaks` times the glyph size of the best unbroken one, with the cost defaulting to 0.4 -- so `Praseo`/`dymium` is taken when it buys a real size step and not when it buys a little. `Breaking.Words` is unchanged, byte for byte, across 1122 measured layouts.
- **`LabelLayout.OpticalCenterOffset` lifts text that sits high.** `Measure` counts the descender on every label whether the text has one or not, so that a fit is judged against the room a tail needs and a live label does not jump a row when a descender appears. The cost is that `TOP` has empty rows under it and looks low when its box is centred. This returns the shift that fixes it -- for *stable* text only, since it depends on the content and would twitch under a label whose words change.

- **`TextSize.Fit` picks the scale too, so the text tracks the pixel as the view zooms.** Auto keeps the scale you asked for, so zooming out leaves the text the same size while the pixels shrink underneath it until neighbouring labels overlap into a grey smear. Fit shrinks with them and then, once even the smallest font no longer fits, draws nothing. Use Fit for a label that belongs to *a pixel* and would be noise once it cannot fit inside one; use Auto or a named size for a label that belongs to *the reader* -- a heading, a readout, anything they are meant to find rather than stumble on. Fit prefers detail over size, so a 7x11 at 1x beats a 5x7 doubled, and the player's `textScale` does not apply to it: what fits inside a pixel is the pixel's business. `Canvas.FitFor(tile, text)` answers in advance, and a scale of 0 means it would not be drawn.
